...
const CHECKS = [
  { id: "focusKeyword", label: "Focus keyword is set and specific (not too generic or too long)", points: 10,
    recommendation: "Pick a specific 2-4 word focus keyword - not a generic single term, and not the full headline." },
  ...
  { id: "slugQuality", label: "URL slug is clean, short, and keyword-focused", points: 5,
  ...
  { id: "internalLinks", label: "Links to a related, already-published post on the same site (same person/company)", points: 5,
    recommendation: "Link the first mention of a person/company already covered elsewhere on the site to that post." },
];
...
// Category-based word-count targets (see README/task spec). Every category has
// a 1200-word FLOOR now (not just "Analysis") - falls back to a generic 1200+
// minimum when no category is supplied or it's unrecognized.
const CATEGORY_WORD_TARGETS = {
  news: [1200, 1500],
  "short news": [1200, 1400],
  shortnews: [1200, 1400],
  business: [1200, 1600],
  company: [1200, 1600],
  startup: [1200, 1600],
...
function wordTargetForCategory(category) {
  ...
  return CATEGORY_WORD_TARGETS[key] || [1200, null];
}
...
// Density = how much of the body text is actually made up of the keyword phrase,
// i.e. (words consumed by all occurrences) / (total words) - NOT (occurrences /
// total words). The latter badly understates density for multi-word keywords:
// an 8-word phrase repeated 5 times in a 550-word article is really ~7% density
// (5*8/550), but counting "occurrences" alone gives 5/550 = 0.9%, which reads as
// perfectly natural. That gap is exactly how a title-length "focus keyword"
// repeated verbatim throughout an article was slipping past this check unflagged.
function keywordDensity(bodyText, keyword) {
  ...
  const wordsConsumed = occurrences * words.length;
  return (wordsConsumed / totalWords) * 100;
}

// A focus keyword should be a short, specific phrase (the prompt asks the AI for
// 2-4 words) - something a person would actually type into Google. A keyword that
// runs much longer than that is really just the headline in disguise, which reads
// as stuffed the moment it's repeated and doesn't function as a search phrase.
const MAX_FOCUS_KEYWORD_WORDS = 5;

function isGenericKeyword(keyword) {
  const words = normalize(keyword).split(/\s+/).filter(Boolean);
  if (words.length === 0) return true;
  if (words.length === 1 && GENERIC_KEYWORDS.has(words[0])) return true;
  if (words.length > MAX_FOCUS_KEYWORD_WORDS) return true;
  return false;
}
 ...
 * @param {boolean} [article.hasInternalLinkCandidates] - true only when the caller
 *   actually checked the site's OTHER published posts and found at least one that
 *   covers a person/company also named in this article (see services/ai.js
 *   attachInternalLinks()). Left false/undefined - e.g. a brand new site with
 *   nothing else published yet, or a caller that never checked - the
 *   "internalLinks" check auto-passes instead of penalizing a link target that
 ...
 */
function scoreSeo({ seoTitle, content, focusKeyword, metaDescription, slug, imageAlt, category, hasInternalLinkCandidates }) {
  ...
  const results = {
    ...
    // Only enforced when the caller confirmed a real link target existed
    // (see the JSDoc note above) - otherwise there's nothing to fairly link to.
    internalLinks: hasInternalLinkCandidates ? $("a[href]").length > 0 : true,
  };
